...
public class OrderEventConsumer {
    ...
    private static final String DEDUPE_PREFIX = "dedupe:payment:order:";
    ...
    public void consume(ConsumerRecord<String, String> record) throws Exception {
        ...
        // Idempotency check — prevent double-charging if event is redelivered
        ...
        log.info("Processing OrderCreated event — orderId={} customerId={} items={}",
                event.getOrderId(), event.getCustomerId(), event.getItems().size());

        // Simulate payment processing
        log.info("Initiating payment for orderId={} customerId={}",
                event.getOrderId(), event.getCustomerId());

        // In a real system: call payment gateway, then publish PaymentConfirmed/PaymentFailed event
